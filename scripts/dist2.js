...
let nextPointId = 0;
// Helper to create a point with a unique id.
function createPoint({ x, y, frequency, amplitude, phase }) {
  return { id: nextPointId++, x, y, frequency, amplitude, phase };
}
...
export default function () {
  ...
  // ---------------------------
  // Setup shader uniforms and points array.
  // ---------------------------
  const uniformSize = 100;
  const points = [
    createPoint({ x: 0.3, y: 0.3, frequency: 10.0, amplitude: 1.0, phase: 0.0 }),
    createPoint({ x: 0.7, y: 0.7, frequency: 15.0, amplitude: 0.8, phase: 1.0 })
  ];

  // Helper functions to produce full-length uniform arrays.
  ...
  const params = {
    blendMode: 'add',
    addPoint: () => {
      const newPoint = createPoint({ x: 0.5, y: 0.5, frequency: 10.0, amplitude: 1.0, phase: 0.0 });
  ...
  function addPointGUI(point) {
    ...
    // Set frequency top-end to 1000.
    folder.add(point, 'frequency', 1, 1000).onChange(updatePoints);
  ...
  function updateCircles() {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      // Convert UV ([0,1]) coordinates to scene coordinates ([-1,1]).
  ...
  // ---------------------------
  // Helper: Given a UV coordinate, return the point (its object, id, and array index) if the pointer is near it.
  // ---------------------------
  ...
  // We'll use the unique id of the point being dragged.
  // ---------------------------
  ...
  renderer.domElement.addEventListener('pointerup', (e) => {
    if (isDragging) {
      ...
      if (result === null) {
        const newPoint = createPoint({ x: latestUV.x, y: latestUV.y, frequency: 10.0, amplitude: 1.0, phase: 0.0 });
        points.push(newPoint);
        updatePoints();
        addPointGUI(newPoint);
        updateCircles();
  ...
  // ---------------------------
  // Initialize the GUI by adding folders for each initial point.
  // ---------------------------
